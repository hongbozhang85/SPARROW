...
FullScreenOpt = 'N'
GeomWin = '800x500'
sideButImaSize = [215,111] # 111*4 = 444, so the width of root ~ 500 bgImage 600x450
setinitialdir = 'C:\Users\dodo\Desktop\soothair_2nd\2_Systems_FullRev'
#####################
...
def filedialogpopup2():
    global FilButEntry
    FilButEntry.delete(0,END)
    filename = tkFileDialog.askopenfilename(defaultextension='ini',filetypes=[('ini','*.ini')], initialdir = setinitialdir)
    if filename:
        FilButEntry.insert(0,filename)
...
def MolButCommand():
    ...
    Checkbutton(mainbodyFrame,text="Jmol and LaTeX: jpg, avi, gif, tex, pdf, div and ps format under /SupInfo/.../",variable=var2,onvalue='YES',offvalue='NO').pack(anchor=W)
...
def ReaButCommand():
    ...
    Label(mainbodyFrame,image=bgImage,width=(winsizewid-sideButImaSize[1])).place(x=0,y=0,relwidth=1,relheight=1)#.pack(fill=X)
    Label(mainbodyFrame,text='Welcome to SPARROW',font=("Courier" ,10, "bold"),relief='flat',width=(winsizewid-sideButImaSize[1])).pack(fill=X)
    ReaHelpText = 'For a give PES, this widget read and output key informations,\nincluding *.params, *.tex, *.jpg and *.avi of all the molecules\non the PES, and sequently generate the input file for PES.nb,\nThermo(both Keq and CTST), Densum, MultiWell and MESMER'
    #Label(mainbodyFrame).pack()
    ...
    Checkbutton(mainbodyFrame,text="Jmol and LaTeX: jpg, avi, gif, tex, pdf, div and ps format under /SupInfo/.../",variable=var3[1],onvalue='YES',offvalue='NO').pack(anchor=W)
